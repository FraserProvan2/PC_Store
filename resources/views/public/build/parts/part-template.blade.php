...
<tr>
    ...
    <td class=" text-right">    
        @if($list_data['purchased'] == false)
            @if($part_id)  
                @if($list_data['gpu_id'] && $part_data->type == "gpu")
                    ...
                    @if($list_data['add_card'] >= 0 && $list_data['add_card'] < 3)
                        <a href="{{ url('add-extra/gpu') }}" class="btn btn-primary"><i class="fa fa-plus" aria-hidden="true"></i></a>
                    @endif

                    @if($list_data['add_card'] <= 3 && $list_data['add_card'] > 0)
                        <a href="{{ url('reduce-extra/gpu') }}" class="btn btn-secondary"><i class="fa fa-minus" aria-hidden="true"></i></a>
                    @endif
            ...
            @else
                <a href="{{ url('list/' . $part) }}" class="btn btn-primary"><i class="fa fa-plus" aria-hibdden="true"></i>&nbsp;Add&nbsp;</a>
